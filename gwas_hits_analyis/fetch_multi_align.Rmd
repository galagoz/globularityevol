...
```{r}
lead_snps = read.table("/data/workspaces/lag/workspaces/lg-ukbiobank/projects/globularity/evolution/results/leadSNPs.txt", header = T)
...
for (i in 1:length(lead_snps_1$rsID)) {
  ...
  m = matrix(NA, nrow = length(jason_object$alignments), ncol = 6)
...
for (i in 1:length(lead_snps_1$rsID)) {
  ...
  tmp_chr = lead_snps_1$chr[i]
  # extract the region covering +-100bp of the variant
  tmp_seq_str = lead_snps_1$pos[i]-10
  tmp_seq_end = lead_snps_1$pos[i]+10
  ...
  m = matrix(NA, nrow = length(jason_object$alignments), ncol = 2)
  tmp_df = as.data.frame(m)
  colnames(tmp_df) = c("species", 
                       "seq")
